...
public class UserEngine extends RootEngine
{
 ...
 /* public UserEngine(Connection conn)
  ...
  public MasterUserBean validate()
  {
    ...
    //Level ID
    tamp = req.getParameter(MasterConstants.FORM_MASTERUSER_LEVEL);
    if(!Utilities.isEmpy(tamp))
    {
    	u.setLevelID(tamp);
    }
    ...
    try
    {
      ...
      if(null!=rs)
      {
        for(int i=0; i<row; i++)
        {
          ...
          lists[i].setLevelID(rs.getString(MasterTable.COL_USERLOGIN_LEVEL));
  ...
  public boolean createMasterUser(MasterUserBean ubn)
  {
    ...
    try
    {
      ...
      stat.setString(4, ubn.getLevelID());
      stat.setString(5, Utilities.dateToString(new Date(), 
          MasterConstants.DATE_DB_MEDIUM_PATTERN));
      stat.setString(6, getUserInSession().getUser());
  ...
  public boolean updateMasterUser(MasterUserBean ubn)
  {
    ...
    try
    {
      ...
      stat.setString(2, ubn.getLevelID());
      stat.setString(3, Utilities.dateToString(new Date(), 
          MasterConstants.DATE_DB_MEDIUM_PATTERN));
      stat.setString(4, super.getUserInSession().getUser());
      stat.setString(5, ubn.getUser());
