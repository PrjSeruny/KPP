...
<%
   ...
   DeathLetterBean[] list = (DeathLetterBean[]) request.getAttribute(TransConstants.DEATHLETTER_LIST);
...
<fieldset>
  <div class="list-con">
    ...
    <% if(stat.equals(TransConstants.DATA_CURRENT)){out.println("DATA AKTIF");}else{ %>
      <a href="<%=Constants.ROOT_PATH%><%=TransConstants.SVT_TRANS_PATH%>?<%=Constants.W%>=<%=TransConstants.TRANS_DEATHLETTER%>&<%=TransConstants.DATA_STAT%>=<%=TransConstants.DATA_CURRENT%>" >DATA AKTIF</a>
    <% } %>
        |
    <% if(stat.equals(TransConstants.DATA_ARCHIEVE)){out.println("ARSIP");}else{ %> 
      <a href="<%=Constants.ROOT_PATH%><%=TransConstants.SVT_TRANS_PATH%>?<%=Constants.W%>=<%=TransConstants.TRANS_DEATHLETTER%>&<%=TransConstants.DATA_STAT%>=<%=TransConstants.DATA_ARCHIEVE%>">ARSIP</a>
    <% } %>
    </div>
    ...
    <a href="<%=Constants.ROOT_PATH%><%=TransConstants.SVT_TRANS_PATH%>?<%=Constants.W%>=<%=TransConstants.TRANS_DEATHLETTER%>&<%=TransConstants.ACT%>=<%=TransConstants.ACT_CREATE%>" class="add" title="Tambah" >Tambah</a>
    ...
    <tr>
        <th><input type="checkbox" class="selAll deathlettids" child="deathlettid"></th> 
        <th>NIK</th>
        <th>Nama</th>
        <th>Pembuat Data</th>
        <th>Tgl Ubah Data</th>
  ...
  %>
          <tr>
            <td align="center">
              <input type="checkbox" class="selChild deathlettid del" parent="deathlettids" name="<%=TransConstants.CHKBOX%>" value="<%=list[i].getNIK()%>">
            </td>
            <td valign="top"><a href="<%=TransConstants.ROOT_PATH%><%=TransConstants.SVT_TRANS_PATH%>?<%=TransConstants.W%>=<%=TransConstants.TRANS_DEATHLETTER%>&<%=TransConstants.ACT%>=<%=TransConstants.ACT_INFO%>&<%=TransConstants.FORM_TRANS_DEATHLETTER_NIK%>=<%=list[i].getNIK()%>"><%=list[i].getNIK()%></a></td>
  ...
  %>
         <tr>
           <td valign="top" align="center" colspan="8">Daftar Kematian tidak ditemukan!</td>
         </tr>
  <%
      }
  %>     
