...
%>
<%
   MasterRegionKecamatanBean[] list = (MasterRegionKecamatanBean[]) request.getAttribute(MasterConstants.MASTERREGION_LIST);
...
<fieldset class="wrapper">
  <div class="form" style="clear:left">
    <form method="post" class="" dst="userlist" id="userlist" 
      action="<%=MasterConstants.ROOT_PATH%><%=MasterConstants.SVT_MASTER_PATH%>?<%=MasterConstants.W%>=<%=MasterConstants.MASTER_REGION_KEC%>&<%=MasterConstants.ACT%>=<%=MasterConstants.ACT_LOOKUP%>">
      ...
      <fieldset>
        ...
        <% 
                ...
                { 
        %>
                   <td align="center"><input class="selChild userid putValues" parent="userids" type="checkbox" name="<%=MasterConstants.FORM_MASTERREGION_KECID%>" value="<%=list[i].getKecamatanID()%>"></td>
         <% 
                 } 
         %>
            ...
            3. tambahkan attribute value dengan isi content yang akan ditempelkan pada halaman yang memanggil fungsi LOOKUP SINGLE, content bisa lebih dari 1 (dipisah oleh semicolon)
             -->
                     <a href="javascript:void(0)" class="putValue" value="<%=list[i].getKecamatanID()%>;<%=list[i].getName()%>">
                       <%=list[i].getKecamatanID()%>
                     </a>
                   <% } %>
                   </td>
        ...
        %>
                 <tr>
                   <td valign="top" align="center" colspan="7">Daftar Kecamatan tidak ditemukan!</td>
                 </tr>
        <%
           }
        %>     
