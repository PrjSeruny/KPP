...
%>
<%
   MasterRegionKelurahanBean[] list = (MasterRegionKelurahanBean[]) request.getAttribute(MasterConstants.MASTERREGION_LIST);
...
<fieldset class="wrapper">
  <div class="form" style="clear:left">
    <form method="post" class="" dst="userlist" id="userlist" 
      action="<%=MasterConstants.ROOT_PATH%><%=MasterConstants.SVT_MASTER_PATH%>?<%=MasterConstants.W%>=<%=MasterConstants.MASTER_REGION_KEL%>&<%=MasterConstants.ACT%>=<%=MasterConstants.ACT_LOOKUP%>">
      ...
      <fieldset>
        ...
        <% 
                ...
                { 
        %>
                   <td align="center"><input class="selChild userid putValues" parent="userids" type="checkbox" name="<%=MasterConstants.FORM_MASTERREGION_KELID%>" value="<%=list[i].getKelurahanID()%>"></td>
         <% 
                 } 
         %>
            ...
            3. tambahkan attribute value dengan isi content yang akan ditempelkan pada halaman yang memanggil fungsi LOOKUP SINGLE, content bisa lebih dari 1 (dipisah oleh semicolon)
             -->
                     <a href="javascript:void(0)" class="putValue" value="<%=list[i].getKelurahanID()%>;<%=list[i].getName()%>">
                       <%=list[i].getKelurahanID()%>
                     </a>
                   <% } %>
                   </td>
        ...
        %>
                 <tr>
                   <td valign="top" align="center" colspan="7">Daftar Kelurahan tidak ditemukan!</td>
                 </tr>
        <%
           }
        %>     
