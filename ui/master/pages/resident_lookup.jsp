...
<fieldset class="wrapper">
  <div class="form" style="clear:left">
    <form method="post" class="" dst="userlist" id="userlist" 
      action="<%=MasterConstants.ROOT_PATH%><%=MasterConstants.SVT_MASTER_PATH%>?<%=MasterConstants.W%>=<%=MasterConstants.MASTER_RESIDENT%>&<%=MasterConstants.ACT%>=<%=MasterConstants.ACT_LOOKUP%>">
      ...
      <fieldset>
        ...
        <% 
		        ...
		        { 
        %>
                   <td align="center"><input class="selChild userid putValues" parent="userids" type="checkbox" name="<%=MasterConstants.FORM_MASTERRESIDENT_NIK%>" value="<%=list[i].getNIK()%>"></td>
         <% 
                 } 
         %>
            ...
            3. tambahkan attribute value dengan isi content yang akan ditempelkan pada halaman yang memanggil fungsi LOOKUP SINGLE, content bisa lebih dari 1 (dipisah oleh semicolon)
             -->
                     <a href="javascript:void(0)" class="putValue" value="<%=list[i].getNIK()%>;<%=list[i].getName()%>">
                       <%=list[i].getNIK()%>
                     </a>
                   <% } %>
                   </td>
        ...
        %>
                 <tr>
                   <td valign="top" align="center" colspan="7">Daftar Penduduk tidak ditemukan!</td>
                 </tr>
        <%
           }
        %>     
